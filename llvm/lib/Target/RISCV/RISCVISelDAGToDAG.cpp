  ...
  case ISD::INTRINSIC_WO_CHAIN: {
    ...
    case Intrinsic::tensor_matmul: {
      printf("In RISCVDagToDagISel select tensor matmul\n");
        SDLoc DL(Node);
        MachineSDNode *tmmul = CurDAG->getMachineNode(RISCV::MLMUL, DL, MVT::i64, Node->getOperand(1), Node->getOperand(2));

        ReplaceUses(SDValue(Node, Node->getNumValues() - 1), SDValue(tmmul, 0));
        CurDAG->RemoveDeadNode(Node);
        tmmul->dump();
        return;
    } 
    case Intrinsic::tensor_log: {
      printf("In RISCVDagToDagISel select tensor log\n");
        SDLoc DL(Node);
        // printf("Node operands:\n");
        // for (unsigned i = 0; i < Node->getNumOperands(); i++) {
        //     Node->getOperand(i).dump();
        // }
        // printf("Done printing node operands\n");
        SDValue Imm = CurDAG->getTargetConstant(0, DL, MVT::i64);

        MachineSDNode *tlog = CurDAG->getMachineNode(RISCV::MLLOG, DL, MVT::i64, Node->getOperand(1), Imm);
        ReplaceUses(SDValue(Node, Node->getNumValues() - 1), SDValue(tlog, 0));
        CurDAG->RemoveDeadNode(Node);
        return;
    }
    case Intrinsic::tensor_add: {
      printf("In RISCVDagToDagISel select tensor add\n");
        SDLoc DL(Node);
        // printf("Node operands:\n");
        // for (unsigned i = 0; i < Node->getNumOperands(); i++) {
        //     Node->getOperand(i).dump();
        // }
        // printf("Done printing node operands\n");
        MachineSDNode *tadd = CurDAG->getMachineNode(RISCV::MLADD, DL, MVT::i64, Node->getOperand(1), Node->getOperand(2));
        ReplaceUses(SDValue(Node, Node->getNumValues() - 1), SDValue(tadd, 0));
        CurDAG->RemoveDeadNode(Node);
        return;
    }
...
#undef CASE_VMXOR_VMANDN_VMOR_OPCODES
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    ...
    case Intrinsic::tensor_load: {
        ...
        // }
        // printf("Done printing node operands\n");
        SDValue Imm = Node->getOperand(3);
        MachineSDNode *tload = CurDAG->getMachineNode(RISCV::MLLOAD, DL, MVT::i64, MVT::Other, {Node->getOperand(2), Imm, Node->getOperand(0)});
        ReplaceUses(SDValue(Node, 0), SDValue(tload, 0));
        ReplaceUses(SDValue(Node, 1), SDValue(tload, 1));
        tload->dump();
        CurDAG->RemoveDeadNode(Node);
        return;
    }
  ...
  case ISD::INTRINSIC_VOID: {
    ...
    case Intrinsic::tensor_store: {
        ...
        SDLoc DL(Node);
        SDValue Imm = CurDAG->getTargetConstant(0, DL, MVT::i64);  // Immediate value 0
        
        // SDValue Ops[] = {Imm, Node->getOperand(3), Node->getOperand(0)};

        // Loop over the oeprands in the node and dump each one
        printf("Node operands:\n");
        for (unsigned i = 0; i < Node->getNumOperands(); i++) {
            Node->getOperand(i).dump();
        }
        ...
        MachineSDNode *tst = CurDAG->getMachineNode(RISCV::MLSTORE, DL, MVT::Other, {Node->getOperand(4), Node->getOperand(2), Imm, Node->getOperand(0)});
        // ReplaceUses(SDValue(Node, Node->getNumValues() - 1), SDValue(tst, 0));
        ReplaceNode(Node, tst);
        CurDAG->RemoveDeadNode(Node);
        tst->dump();
        return;
    }
