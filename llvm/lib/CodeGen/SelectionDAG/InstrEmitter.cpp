...
void InstrEmitter::AddOperand(MachineInstrBuilder &MIB,
  ...
  if (Op.isMachineOpcode()) {
    ...
    MIB.addTargetIndex(TI->getIndex(), TI->getOffset(), TI->getTargetFlags());
      ...
      //mlisa
      // printf("Massive else if add operand chain and glue error: \n");
      // Op->dump();
      // fflush(stdout);
    assert(Op.getValueType() != MVT::Other &&
           Op.getValueType() != MVT::Glue &&
...
EmitMachineNode(SDNode *Node, bool IsClone, bool IsCloned,
  ...
  //mlisa
  // printf("NumMI Operands: %d\n", NodeOperands + NumResults);
  // printf("II Num Operands: %d\n", II.getNumOperands());
  //
  // printf("looping over the MI operands: \n");
  //   for (unsigned i = 0; i < Node->getNumOperands(); i++) {
  //       printf("Operand %d: ", i);
  //       Node->getOperand(i).getNode()->dump();
  //   }
